...
namespace TheToolsmiths.Ddl.Writer.OutputWriters
{
    ...
    {
        public static async Task<Result> WriteOutputToConsole(PipeReader pipeReader)
        {
            try
            {
                await HandleWriteOutputToConsole(pipeReader).ConfigureAwait(false);
            }
            catch (Exception e)
            {
        ...
        private static async Task HandleWriteOutputToConsole(PipeReader pipeReader)
        {

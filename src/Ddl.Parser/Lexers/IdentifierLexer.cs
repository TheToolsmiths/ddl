...
{
    internal static class IdentifierLexer
    {
        public static void HandleIdentifierPhase(ref LexerSequenceReadState readState)
        {
            ...
            while (readState.HasCurrent)
            {
                ...
                if (CharHelpers.IsValidIdentifierChar(readState.CurrentChar) == false)
                {
            ...
            // but we havent finished reading the token.
            {
                ...
                var memory = readState.GetStageScratchMemory(identifierChars.Length);

                identifierChars.CopyTo(memory.Span);
